...
const Menu = () => {
    ...
    const getAllData = async () => {
        return await axios.get('http://localhost:3008/menu/')
    ...
    const updateUser = async (e) => {
        try {
          const response = await axios.put('http://localhost:3100/menu/update/'+id, dataUp);
          window.location.href = '/menu'
    ...
    useEffect(() => {
        id ? getDataById().then((result) => setDataUp((result))) : <></>
    }, [id])
        ...
        <>
            <Dashboard page='/menu'>
                ...
                <div className='w-full flex justify-end'>
                    <button onClick={() => setModalAdd(true)} className='rounded-md bg-black px-3 py-1 text-white cursor-pointer'>Add Menu</button>
                </div>
                <div className='flex w-[100%] gap-12 flex-wrap text-black'>
                    {
                        dataMenu.map((e, i) => (
                            <div key={i} className="relative flex flex-col bg-white shadow-sm border border-slate-200 rounded-lg w-65 text-black">
                                ...
                                <div className="px-4 pb-4 w-full flex items-end justify-between gap-2">
                                    ...
                                    <div className='flex w-fit gap-2'>
                                        ...
                                        <button className='px-2 rounded-md py-2 cursor-pointer bg-black text-white' onClick={() => handleDelete(e.id)}><FaTrash /></button>
                                    </div>
                                </div>
                            </div>
                        ))
